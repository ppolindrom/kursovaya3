...
from src.unit import load_data, get_last, sorted_operation, masked_number


@pytest.fixture
def test_data():
    return load_data(os.path.join('tests', 'test_operations.json'))
...
def test_load_data():
    ...
    assert load_data(os.path.join('tests', 'test_operations.json')) == expected_result
...
def test_sorted(test_data):
    ...
    assert sorted_operation(get_last(test_data)) == expected_result

def test_masked_number(test_data):
    assert masked_number('МИР 5211274343558469') == "МИР 5211 27** ****8469"